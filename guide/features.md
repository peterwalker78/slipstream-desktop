...
- **Run a command** (Super+R): a command line, like Windows' Run box. What you type runs as written, arguments and all, even when an app of that name is installed — which is the thing a tapped Super can't do. The commands you ran before are offered back, so an empty Run box is your recent commands.
